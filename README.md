...
```bash
git clone https://github.com/IndigoHiro/udacity_drlnd_p2_reacher.git
cd udacity_drlnd_p2_reacher/python
pip install .
```
...
5. Place the downloaded file in repository's root directory, and unzip (or decompress) the file. 

### Instructions



Follow the instructions in `ddpg/Tennis.ipynb` to get started with training the agent! 
The notebook uses a DDPG agent implemented in `ddpg/ddpg_agent.py`. 
You can also see how an already trained model works without training the agent yourself. 
`ddpg/checkpoint.pth` contains the trained model by the DDPG agent. 
